    ...
    unsigned       queue_index)
{
    ...
    // Diagnostic Vulkan seam test (2026-04-03): keep a non-NULL dummy
    // negotiation interface on the runtime struct too. Bootstrap already does
    // this because at least one core crashes when it blindly dereferences a
    // NULL negotiation pointer during early bring-up. If LRPS2 has the same
    // assumption during context_reset, this changes the failure shape without
    // touching set_image / zero-copy semantics.
...
var (
	...
	diagDumpCachedCount  int64 // one-shot cached frame dump for offline format checks
)
...
func dumpCachedFrameVariants(label string, pixels []byte, w, h uint32) error {
	if len(pixels) == 0 || w == 0 || h == 0 {
		return fmt.Errorf("empty dump input w=%d h=%d len=%d", w, h, len(pixels))
	}
	baseDir := "/tmp/cloudplay-vulkan-dumps"
	if err := os.MkdirAll(baseDir, 0o755); err != nil {
		return err
	}
	rawPath := fmt.Sprintf("%s/%s.rgba", baseDir, label)
	if err := os.WriteFile(rawPath, pixels, 0o644); err != nil {
		return err
	}
	rgbaImg := image.NewRGBA(image.Rect(0, 0, int(w), int(h)))
	copy(rgbaImg.Pix, pixels)
	rgbaFile, err := os.Create(fmt.Sprintf("%s/%s-rgba.png", baseDir, label))
	if err != nil {
		return err
	}
	if err := png.Encode(rgbaFile, rgbaImg); err != nil {
		rgbaFile.Close()
		return err
	}
	if err := rgbaFile.Close(); err != nil {
		return err
	}
	bgraImg := image.NewRGBA(image.Rect(0, 0, int(w), int(h)))
	for i := 0; i+3 < len(pixels) && i+3 < len(bgraImg.Pix); i += 4 {
		bgraImg.Pix[i+0] = pixels[i+2]
		bgraImg.Pix[i+1] = pixels[i+1]
		bgraImg.Pix[i+2] = pixels[i+0]
		bgraImg.Pix[i+3] = pixels[i+3]
	}
	bgraFile, err := os.Create(fmt.Sprintf("%s/%s-bgra.png", baseDir, label))
	if err != nil {
		return err
	}
	if err := png.Encode(bgraFile, bgraImg); err != nil {
		bgraFile.Close()
		return err
	}
	if err := bgraFile.Close(); err != nil {
		return err
	}
	return nil
}
...
type Provider struct {
	...
	// usesWaitSync is set to true the first time go_wait_sync_index is called.
	// Cores that use wait_sync_index (Dolphin) get their blit done there;
	// cores that don't (LRPS2) rely on go_unlock_queue for blits/readback.
	...
	if p.ctx.externalHandles {
		...
		if cached != nil {
			...
			if drn%60 == 1 && len(cached) >= 16 {
				...
				if nonZero > 0 && atomic.CompareAndSwapInt64(&diagDumpCachedCount, 0, 1) {
					label := fmt.Sprintf("readframe-%d-%dx%d", drn, w, h)
					if err := dumpCachedFrameVariants(label, cached, w, h); err != nil {
						log.Printf("[cloudplay diag] dumpCachedFrameVariants failed: %v", err)
					} else {
						log.Printf("[cloudplay diag] dumped cached frame variants label=%s dir=/tmp/cloudplay-vulkan-dumps", label)
					}
				}
			}
			return cached, nil
...
func (p *Provider) ReadFrameZeroCopy(w, h uint32) (fd int, size uint64, err error) {
	...
	if !alreadyBlitted {
		log.Printf("[cloudplay diag] ReadFrameZeroCopy: sync BlitFrom seq=%d image=%p requested=%dx%d layout=%d zc=%dx%d waitSems=%d srcQF=%d dstQF=%d", imgSeq, img, w, h, int(layout), zcW, zcH, len(waitSems), uint32(waitSrcQF), p.ctx.QueueFamily)
		if err = p.zeroCopy.BlitFromWait(img, layout, waitSrcQF, waitSems); err != nil {
			return -1, 0, err
		}
	...
	if repeatedImage && zcW > 0 && zcH > 0 {
		if atomic.CompareAndSwapInt64(&diagRepeatedSetProbe, 0, 1) {
			...
			if tempErr != nil {
				...
				if tempErr = tempZC.BlitFromWait(image.create_info.image, image.image_layout, srcQF, waitSemsCopy); tempErr != nil {
...
func go_wait_sync_index(handle unsafe.Pointer) {
	...
	// Mark that this core uses wait_sync_index so go_unlock_queue can
	// skip its expensive GPU sync (blit + readback) — we handle it here.
	if !p.usesWaitSync {
		p.usesWaitSync = true
	}
	...
	if p.ctx.externalHandles {
		...
		if p.zeroCopyAvailable() {
			...
			if zcErr == nil {
				...
				blitErr := p.zeroCopy.BlitFromAsync(img, layout, waitSrcQF)
				if blitErr == nil {
					p.mu.Lock()
					p.zeroCopyBlitDone = true
					p.mu.Unlock()
...
func go_unlock_queue(handle unsafe.Pointer) {
	...
	// Blit + readback are only needed here for cores that do NOT call
	// wait_sync_index (e.g. LRPS2).  Cores that DO call wait_sync_index
	// (Dolphin) get their blit done there — doing it here too adds 2-4ms
	// of synchronous GPU stalls per frame, killing 60fps.
	if !p.usesWaitSync {
		// For cores that use lock_queue/unlock_queue instead of wait_sync_index
		// (e.g. LRPS2), the zero-copy buffer never gets refreshed after the
		// initial one-shot blit in ReadFrameZeroCopy.  Fix: blit the current
		// image into the exportable ZeroCopyBuffer here so CUDA/NVENC sees
		// fresh pixels each frame.
		if p.ctx.externalHandles && img != nil && p.zeroCopyAvailable() {
			w := uint32(640)
			h := uint32(448)
			if zcW > 0 && zcH > 0 {
				w, h = zcW, zcH
			...
			if zcErr == nil {
				...
				if blitErr == nil {
					...
					p.zeroCopyBlitDone = true
					p.mu.Unlock()
					bn := atomic.AddInt64(&frameCount, 1)
					if bn <= 5 || bn%300 == 0 {
						log.Printf("[cloudplay] go_unlock_queue: zero-copy BlitFrom submitted frame=%d srcQF=%d", bn, uint32(waitSrcQF))
					}
				} else {
					bn := atomic.AddInt64(&frameCount, 1)
					if bn <= 5 || bn%60 == 0 {
						log.Printf("[cloudplay] go_unlock_queue: zero-copy BlitFrom err=%v, falling back to CPU readback", blitErr)
					}
				}
			}
		}

		// CPU readback fallback: some cores use lock/unlock instead of
		// wait_sync_index, so also populate cachedPixels here.
		if p.ctx.externalHandles && img != nil {
			w := uint32(640)
			h := uint32(448)
			if zcW > 0 && zcH > 0 {
				w, h = zcW, zcH
			...
			if p.fc == nil || p.fc.width != w || p.fc.height != h {
				...
				fc, err := NewFrameCapture(p.ctx, w, h)
				if err != nil {
					log.Printf("[cloudplay diag] go_unlock_queue cache-create failed: %v", err)
				} else {
					p.fc = fc
				}
			}
			if p.fc != nil {
				pixels, err := p.fc.Readback(img, layout)
				if err != nil {
					log.Printf("[cloudplay diag] go_unlock_queue cache-readback failed: %v", err)
					...
					n := atomic.AddInt64(&frameCount, 1)
					if n <= 5 || n%300 == 0 {
						log.Printf("[cloudplay] go_unlock_queue: CPU Readback %d bytes, first16=%v", len(cp), cp[:min(16, len(cp))])
					}
				}
			}
		}
